...
class AgentExecutor(Chain):
    ...
    def _perform_agent_action(
        ...
        if agent_action.tool in name_to_tool_map:
            ...
            if __ctaudit_unreachable__:  # [ctaudit] resolved dynamic dispatch -> 2 targets
                __ctaudit_ret = PythonREPLTool._run(agent_action.tool_input, verbose=self.verbose, color=color)
                observation = __ctaudit_ret
                __ctaudit_ret = PythonAstREPLTool._run(agent_action.tool_input, verbose=self.verbose, color=color)
                observation = __ctaudit_ret
        else:
            tool_run_kwargs = self._action_agent.tool_run_logging_kwargs()
