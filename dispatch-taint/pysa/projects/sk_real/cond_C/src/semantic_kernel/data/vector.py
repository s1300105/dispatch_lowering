...
from typing import TYPE_CHECKING, Annotated, Any, ClassVar, Final, Generic, Literal, Protocol, TypeVar, overload, runtime_checkable
    ...
    def _create_kernel_function(
        ...
        async def search_wrapper(**kwargs: Any) -> Sequence[str]:
            ...
            if __ctaudit_unreachable__:  # [ctaudit] resolved dynamic dispatch -> 1 targets
                ...
                __ctaudit_ret = default_dynamic_filter_function(kwargs, query, inner_options.filter, inner_options, results, msg, self, search_type, options, parameters, filter_update_function, return_parameter, function_name, description, string_mapper, update_func)
            ...
            match search_type:
                case SearchType.VECTOR:
                    try:
                        if __ctaudit_unreachable__:  # [ctaudit] resolved dynamic dispatch -> 1 targets
                            from semantic_kernel.connectors.in_memory import InMemoryCollection
                            __ctaudit_ret = InMemoryCollection._parse_and_validate_filter(kwargs, __ctaudit_ret, query, inner_options.filter, inner_options, results, msg, self, search_type, options, parameters, filter_update_function, return_parameter, function_name, description, string_mapper, update_func)
